...
def adv_emb_attack(
    ...
    for i, (images, image_paths) in enumerate(tqdm(wm_loader)):
        ...
        for img_adv, image_path in zip(images_adv, image_paths):
            image_name = os.path.basename(image_path).split('_')[0].split('.')[0] +'/' + os.path.basename(image_path).split('.')[0]+ '_adv_' + encoder+ '_' +str(strength) +'.jpg'
            print(image_name)
            os.makedirs(os.path.join(output_path, os.path.basename(image_path).split('_')[0].split('.')[0] ), exist_ok=True)
